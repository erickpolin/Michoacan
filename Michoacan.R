...
ConcMichoacan$Nhog <- 1

#DECILES 

#Attaching the data frame
attach(ConcMichoacan) #this is for not writing the name of the data frame and the $ ever time

#Sort Conc according to INGCOR, FOLIOVIV, FOLIOHOG
ConcMichoacan<- orderBy (~+ing_cor+folioviv+foliohog, data=ConcMichoacan) #this give us the households sorted by total income

#Adding the values of the expansion FACTOR. The sum is 34 million, which is the number of households in the country.
tot_hogares<-sum(ConcMichoacan$factor,to.data.frame=TRUE)
...
ConcMichoacan$tam_dec<-tam_dec

# Creating Deciles of Income 

ConcMichoacan$MAXT<-ConcMichoacan$ing_cor #vamos a crear en esta base la variable MAXT que es una copia de la columna de ingresos

ConcMichoacan<-ConcMichoacan[with(ConcMichoacan, order(rank(MAXT))),]  #lo que hicimos aqu? fue reordenar la base con respecto a MAXT. Cosa que en realidad, ya estaba.

ConcMichoacan$ACUMULA<-cumsum(ConcMichoacan$factor) #aqu? creamos una variable de suma acumulada del FACTOR de viviendas.
...
for(i in 1:9)
{
  a1<-ConcMichoacan[dim(ConcMichoacan[ConcMichoacan$ACUMULA<tam_dec*i,])[1]+1,]$factor
  ConcMichoacan<-rbind(ConcMichoacan[1:(dim(ConcMichoacan[ConcMichoacan$ACUMULA<tam_dec*i,])[1]+1),],
                             ConcMichoacan[(dim(ConcMichoacan[ConcMichoacan$ACUMULA<tam_dec*i,])[1]+1):dim(ConcMichoacan[1])[1],])
  
  b1<-tam_dec*i-ConcMichoacan[dim(ConcMichoacan[ConcMichoacan$ACUMULA<tam_dec*i,])[1],]$ACUMULA
  ConcMichoacan[(dim(ConcMichoacan[ConcMichoacan$ACUMULA<tam_dec*i,])[1]+1),]$factor<-b1
  ConcMichoacan[(dim(ConcMichoacan[ConcMichoacan$ACUMULA<tam_dec*i,])[1]+2),]$factor<-(a1-b1)
}

#aqu? estamos creando otra variable de suma acumulada del n?mero de hogares
ConcMichoacan$ACUMULA2<-cumsum(ConcMichoacan$factor)

#aqu? estamos creando una variable que se llama decil que solo tiene ceros
ConcMichoacan$DECIL<-0
...
ConcMichoacan[(ConcMichoacan$ACUMULA2<=tam_dec),]$DECIL<-1
...
for(i in 1:9)
{
  ConcMichoacan[((ConcMichoacan$ACUMULA2>tam_dec*i)&(ConcMichoacan$ACUMULA2<=tam_dec*(i+1))),]$DECIL<-(i+1)
}

# a lo que le qued? cero (que es la ?ltima observaci?n), ponle el decil 10
ConcMichoacan[ConcMichoacan$DECIL%in%"0",]$DECIL<-10

setwd("C:/Users/Erick/Dropbox/GIC/GITHUB2018/GIC/Michoacan")
write.dbf(ConcMichoacan,file="ConcMichoacan2010.dbf")
...
setwd("C:/Users/Erick/Dropbox/GIC/GITHUB2018/GIC/Michoacan")
write.dbf(ConcMichoacan,file="ConcMichoacan2018.dbf")

rm(list=ls())



########## CreaciÃ³n tablas ingresos total 2010 #######
...
#reading the data
setwd("C:/Users/Erick/Dropbox/GIC/GITHUB2018/GIC/Michoacan")
Conc2010<-read.dbf("ConcMichoacan2010.dbf",as.is = T)
...
write.dbf(c_DECIL_ES,file = "Michoacan por fuente por DECIL estimaciones 2010.dbf")
write.dbf(c_DECIL_SE,file = "Michoacan por fuente por DECIL errores standard 2010.dbf")
write.dbf(c_DECIL_CV,file = "Michoacan por fuente por DECIL CV 2010.dbf")
write.dbf(c_DECIL_LI,file = "Michoacan por fuente por DECIL LI 2010.dbf")
write.dbf(c_DECIL_ES,file = "Michoacan por fuente por DECIL LS 2010.dbf")
...
write.dbf(c_DECIL_ES,file = "Michoacan INDIGENA por fuente por DECIL estimaciones 2010.dbf")
write.dbf(c_DECIL_SE,file = "Michoacan INDIGENA por fuente por DECIL errores standard 2010.dbf")
write.dbf(c_DECIL_CV,file = "Michoacan INDIGENA por fuente por DECIL CV 2010.dbf")
write.dbf(c_DECIL_LI,file = "Michoacan INDIGENA por fuente por DECIL LI 2010.dbf")
write.dbf(c_DECIL_ES,file = "Michoacan INDIGENA por fuente por DECIL LS 2010.dbf")
...
write.dbf(c_DECIL_ES,file = "Michoacan NO por fuente por DECIL estimaciones 2010.dbf")
write.dbf(c_DECIL_SE,file = "Michoacan NO por fuente por DECIL errores standard 2010.dbf")
write.dbf(c_DECIL_CV,file = "Michoacan NO por fuente por DECIL CV 2010.dbf")
write.dbf(c_DECIL_LI,file = "Michoacan NO por fuente por DECIL LI 2010.dbf")
write.dbf(c_DECIL_ES,file = "Michoacan NO por fuente por DECIL LS 2010.dbf")

rm(list = ls())

########## CreaciÃ³n tablas ingresos total 2018 ########
...
#reading the data
setwd("C:/Users/Erick/Dropbox/GIC/GITHUB2018/GIC/Michoacan")
Conc2018<-read.dbf("ConcMichoacan2018.dbf",as.is = T)
...
write.dbf(c_DECIL_ES,file = "Michoacan Ingresos por fuente por DECIL estimaciones 2018.dbf")
write.dbf(c_DECIL_SE,file = "Michoacan Ingresos por fuente por DECIL errores standard 2018.dbf")
write.dbf(c_DECIL_CV,file = "Michoacan Ingresos por fuente por DECIL CV 2018.dbf")
write.dbf(c_DECIL_LI,file = "Michoacan Ingresos por fuente por DECIL LI 2018.dbf")
write.dbf(c_DECIL_ES,file = "Michoacan Ingresos por fuente por DECIL LS 2018.dbf")
...
write.dbf(c_DECIL_ES,file = "Michoacan INDIGENA Ingresos por fuente por DECIL estimaciones 2018.dbf")
write.dbf(c_DECIL_SE,file = "Michoacan INDIGENA Ingresos por fuente por DECIL errores standard 2018.dbf")
write.dbf(c_DECIL_CV,file = "Michoacan INDIGENA Ingresos por fuente por DECIL CV 2018.dbf")
write.dbf(c_DECIL_LI,file = "Michoacan INDIGENA Ingresos por fuente por DECIL LI 2018.dbf")
write.dbf(c_DECIL_ES,file = "Michoacan INDIGENA Ingresos por fuente por DECIL LS 2018.dbf")
...
write.dbf(c_DECIL_ES,file = "Michoacan NO Ingresos por fuente por DECIL estimaciones 2018.dbf")
write.dbf(c_DECIL_SE,file = "Michoacan NO Ingresos por fuente por DECIL errores standard 2018.dbf")
write.dbf(c_DECIL_CV,file = "Michoacan NO Ingresos por fuente por DECIL CV 2018.dbf")
write.dbf(c_DECIL_LI,file = "Michoacan NO Ingresos por fuente por DECIL LI 2018.dbf")
write.dbf(c_DECIL_ES,file = "Michoacan NO Ingresos por fuente por DECIL LS 2018.dbf")
...
Deciles2010<-read.dbf("Michoacan por fuente por DECIL estimaciones 2010.dbf")
names(Deciles2010)[1]<-c("ingcor2010")

Deciles2018<-read.dbf("Michoacan Ingresos por fuente por DECIL estimaciones 2018.dbf")
...
deciles2010No<-read.dbf("Michoacan NO por fuente por DECIL estimaciones 2010.dbf")
names(deciles2010No)[1]<-c("No2010")

deciles2018No<-read.dbf("Michoacan NO Ingresos por fuente por DECIL estimaciones 2018.dbf")
...
deciles2010indigena<-read.dbf("Michoacan INDIGENA por fuente por DECIL estimaciones 2010.dbf")
names(deciles2010indigena)[1]<-c("Indigena2010")

deciles2018indigena<-read.dbf("Michoacan INDIGENA Ingresos por fuente por DECIL estimaciones 2018.dbf")
